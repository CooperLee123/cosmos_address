...
func mnemonicKeyCommand() *cobra.Command {
	fmt.Println("--mnemonicKeyCommand--")
	cmd := &cobra.Command{
		Use:   "mnemonic",
...
func runMnemonicCmd(cmd *cobra.Command, args []string) error {
	...
	if userEntropy {
		...
		fmt.Println("inputEntropy:",inputEntropy)
		if err != nil {
			return err
		}
		...
		fmt.Println("entropySeed:",entropySeed)
